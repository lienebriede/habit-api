    ...
    def validate(self, data):
        ...
        if predefined_habit1 and custom_habit1:
            raise serializers.ValidationError(
                "You can only choose either a predefined habit or a custom "
                "habit for Habit 1, not both."
            )

        if predefined_habit2 and custom_habit2:
            raise serializers.ValidationError(
                "You can only choose either a predefined habit or a custom "
                "habit for Habit 2, not both."
            )

        if not (predefined_habit1 or custom_habit1):
            raise serializers.ValidationError(
                "You must provide either a predefined habit or a custom habit "
                "for Habit 1."
            )
        ...
        if (
            (predefined_habit1 and predefined_habit2 and
             predefined_habit1 == predefined_habit2) or
            (custom_habit1 and custom_habit2 and
             custom_habit1 == custom_habit2) or
            (predefined_habit1 and custom_habit2 and
             predefined_habit1.name == custom_habit2) or
            (predefined_habit2 and custom_habit1 and
             predefined_habit2.name == custom_habit1)
        ):
            raise serializers.ValidationError(
                "Habit1 and Habit2 cannot be the same."
            )

        # Ensure both habit fields are filled (predefined or custom)
        if not (
            (predefined_habit1 or custom_habit1) and
            (predefined_habit2 or custom_habit2)
        ):
            raise serializers.ValidationError(
                "Both habit fields cannot be empty. "
                "Provide either predefined or custom habits."
            )
        ...
        if existing_habit_stacks.exists():
            raise serializers.ValidationError(
                "This habit stack already exists."
                )
...
class HabitStackingLogSerializer(serializers.ModelSerializer):
    ...
    def get_streak_message(self, obj):
        ...
        for log in user_logs:
            if log.date == today or log.date == today - timedelta(
                days=streak_count
            ):
                streak_count += 1
                today = log.date
